...
public class Person implements Serializable{
    ...
    @Nullable
    public static Person[] parseFromXML() {
        try {
            ...
            Person[] persons = new Person[nList.getLength()];
            for (int temp = 0; temp < nList.getLength(); temp++) {
                ...
                if (nNode.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    persons[temp] = new Person(firstName,lastName, telephoneNumber,mail,workPlace, workExperience);
                }
            }
            return persons;
        } catch (Exception e) {
    ...
    public static String parseToXML(Person[] persons , int mask) {
        String result = "";
        if ((mask & XMLBEGIN) == XMLBEGIN) {
            result += XMLBegin();
        }
